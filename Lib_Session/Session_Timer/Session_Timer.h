...
enum Hz_or_s {FREQUENCY = 0, PERIOD};

// create a pointer type which points to a void function
typedef void (*GeneralFunction) ();

class Session_Timer //: public Session
{
	static void _isr0(); // interrupt system register
	static Session_Timer *_instance; // pointer to instance of Timer
	void handleInterrupt(); // called routine when running _isr0()

	public:
		Session_Timer(void); //ctor
		Session_Timer(const char* name); //ctor
		~Session_Timer(void); //dtor

		void set_name(const char* name);
		const char* get_name();
		void print_name();

		void set_us_period(int us_period);
		int get_us_period();
		...
		void add(GeneralFunction f, int time_style, double time); // adds a function to time trigger
	...
	protected:
		...
		int _us_period; // period in microseconds
		int _tasks_in; // number of functions triggered by the timer

		GeneralFunction *_Functions; // array where are stored functions to time trigger
		double *_FunctionsCounters; // time counters of functions to time trigger
		double *_FunctionsPeriods; // time periods of functions to time trigger
};
#endif
///////////////////////////////////////////////////////////END
///////////////////////////////////////////////////////////END
